...
\ref{fig:simple_uml}). Input data to the study are also stored in the repository, and are
...
\emph{Pipeline} object, and referencing the method by name in a FilesetSpec
or FieldSpec in the class' data specification table. Pipeline objects are thin
wrappers around Nipype workflows, which manage the namespaces of the
workflow nodes, and should be created in constructor methods using the
\emph{pipeline} method of the Study class (Figure \ref{fig:pipeline_constructor}).
...
% Need to introduce and explain new syntax
...
At initialisation, a Pipeline is given a name, which must be unique amongst the
pipelines that can be constructed by the Study. A description of the pipeline and
the list of scientific references

Study class Inputs and outputs refer to entries in the data specification table and
...
% End reworked syntax explanation
...
to construct a base Pipeline, apply name_maps and return the
...
to select input datasets from the repository (Figure \ref{fig:study_application}).
...
implementing a small set of methods (Table \ref{tbl:abstract-repo-methods}).
...
fields \\
\emph{get\_fileset(Fileset)} & Caches file-set (if necessary) and returns the file path. \\
\emph{get\_field(Field)} & Retrieves and returns the value of the field from
the repository \\
\emph{put\_fileset(Fileset)} & Inserts file-set into the repository and updates cache. \\
...
\emph{Study.data(name{[}, name,...{]})} method (Figure \ref{fig:simple_uml}). The first

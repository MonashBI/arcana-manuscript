...
These are the notes I have for my abstract

\begin{itemize}
\item The heterogeneous collection of neuroimaging analysis tools, parameters, styles makes it difficult to design comprehensive workflows
\item Complex workflows in a flexible extensible manner, reusable
\item portable between different data storage solutions/scale of studies, separation between design and application yet programmatic
\item Easily updatable, switchable between tools/versions
\end{itemize}
...
\begin{figure}
\lstinputlisting[basicstyle=\footnotesize,firstline=2, lastline=59]{listings/example_study.py}
...
which are thin wrappers around Nipype workflows to handle input/output
...
methods, typically in subclasses and multi-studies.

\begin{figure}
\lstinputlisting[firstline=61, basicstyle=\footnotesize]{listings/example_study.py}
...
Within a pipeline constructor, the Pipeline object is initialised by the \emph{pipeline} method
of the Study (Figure \ref{fig:pipeline_constructor}). At initialisation, each pipeline is given a name,
...
The \emph{add(name, interface)} method is used to add a node to a pipeline, which takes
...
table or study input then an conversion nodes are implicitly connected to the pipeline.
Inputs and outputs that are conditional on parameters of the study can be set outside of
the \emph{add} method using the \emph{connect\_input} or
\emph{connect\_output} methods, respectively.

Internal connections between nodes within a pipeline are specified by a dictionary
passed to the \emph{connect} keyword argument of the receiving node. The
dictionary keys are the names of the receiving traits and the values are 2-tuples
...
modules \citep{furlani_modules:_1991}, to ensure the required software
packages are available. Similarly, the expected memory requirements and
wall time for the node execution should be provided to the keyword args
\emph{memory}, and \emph{wall\_time} so they can be inserted into HPC
scheduler job scripts if required.
...
passed to the \emph{joinsource} keyword to join over subjects or visits, respectively.
...
The subject and visit IDs are directly accessible as input fields of the pipeline
...
tested with \emph{branch(name)} in the case of boolean switches and
\emph{branch(name, ref\_value)} in the case of string switches.
...
\begin{figure}
\lstinputlisting[basicstyle=\footnotesize]{listings/example_application.py}
...
The software packages installed on the system are detected and managed
by the Environment module. There are currently two types of available Environment modules:
\emph{StaticEnvironment} and \emph{ModulesEnvironment}.

The \emph{StaticEnvironment} module doesn't perform any management, and instead
only checks the existing environment for appropriate software versions before running
the analysis.

The \emph{ModulesEnvironment} module can be used if environment
modules \citep{furlani_modules:_1991} are installed on the system (typical in many
HPC environments). In this case, modules are loaded before a workflow node is run,
...
module names installed on the system to requirements required by the Study can be
can be passed to the \emph{packages\_map} keyword argument of the ModulesEnvironment
at initialisation. Likewise non-standard versions for packages can be mapped onto requirements
using the \emph{versions\_map} keyword argument.
...
\emph{T2starT1Study} MultiStudy class to combine analysis of T2* and
...
The \emph{T2starT1Study} class implements an algorithm to generate
...
registered to the T1-weighted acquisition \citep{ward_combining_2018}.
(Figure \ref{fig:t2star_t1_study}).
...
manuscript\emph{, ArcanaPaper}, (Supplementary material). Methods are
implemented in ArcanaPaper to generate and display the derivative
images in Figure \ref{fig:veins}, \ref{fig:fa_adc} and \ref{fig:tractography}.
...
Methods for Figure \ref{fig:veins}, \ref{fig:fa_adc}, and \ref{fig:tractography} in
...
Figure \ref{fig:veins} displays composite vein images for the healthy
...
\begin{figure*}
  ...
  generated (\emph{bottom row}) by thresholding the composite
  vein image.
...
node within its own container if required to take advantage of

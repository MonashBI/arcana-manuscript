...
standard, a \emph{BidsInput} or \emph{BidsAssocInput} can be
provided to the \emph{bids} keyword arg of the spec. BidsInput
specifies a primary scan in the BIDS standard using its type, modality
and optional run number. BidsAssocInput is used to select
associated files, such as field maps and diffusion encoding matrices.
When passing a BidsInput to a FilesetSpec the \emph{run} keyword
...
upstream node's output interface (output connections are implied by input connections to other nodes).
...
to the pipeline by the framework to perform the required conversion. Pipeline inputs and outputs of
...
There are no restrictions between mapping input and derivative data
...
the dictionary keys are in the sub-study namespace and the dictionary
values are in the global namespace.
...
``directory'' format, which are encapsulated by \emph{XnatRepo},
\emph{BidsRepo}, and \emph{DirectoryRepo} classes
respectively.
...
Selection criteria are specified by list of \emph{FilesetInput} and
\emph{FieldInput} objects provided to the \emph{inputs} argument of
...
Arcana: \emph{SingleProc}, \emph{MultiProcProc},
\emph{SlurmProc}, which wrap the

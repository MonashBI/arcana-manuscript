...
\item portable between different data storage solutions/scale of studies, separation between design and application yet programmatic, prototype locally and deploy on cluster
\item Easily updatable, switchable between tools/versions
\item Data specification, like make, dependencies are resolved by framework
\item Modular Repository, Processor, Environment and input matching
\item supports BIDS
\item provenance, checks for updates to find stale data and reprocesses
\item we have created a framework that handles all the tedious aspects and allows researchers to focus on the analysis design
\item it is flexible to allow researchers to collaborate
\end{itemize}

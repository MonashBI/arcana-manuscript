...
%\includegraphics[width=6.27083in,height=3.94444in]{media/image36.png}

Listing - Example application of Study class to a dataset stored in an XNAT
...
\caption{Storage locations of derivative and provenance data for each repository
...
parameterisations. Where `...' = \emph{derivatives}
directory/XNAT session, \emph{subj} = subject ID, \emph{vis} = visit ID,
\emph{study} = name of the Study instance, and \emph{pl} = name
of pipeline.}
\label{tbl:repo-structure}       % Give a unique label
% For LaTeX tables use
\begin{tabular}{llll}
\hline\noalign{\smallskip}
\textbf{Type} & Plain-directory & BIDS & XNAT  \\
\noalign{\smallskip}\hline\noalign{\smallskip}
\textbf{Deriv.} & /\emph{subj}/\emph{vis}/\emph{study} & /derivatives/\emph{study}/\emph{subj}/\emph{vis} & /\emph{subj}/\emph{vis}\_\emph{study} \\
\textbf{Fields} & \D/\_\_fields\_\_.json & \D/\_\_fields\_\_.json & ``custom vars.'' \\
\textbf{Prov.} & \D/\_\_prov\_\_/\emph{pl}.json & \D/\_\_prov\_\_/\emph{pl}.json & \D/\_\_prov\_\_/\emph{pl}/prov\emph{.}json \\
\noalign{\smallskip}\hline
\end{tabular}
\end{table}

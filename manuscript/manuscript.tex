...
At the heart of each Study class is the data specification attribute, which
...
represent integer, floating point, date-time, or character string
...
which must be unique amongst the pipelines constructed by the Study given the any combination of
...
names of entries in the data specification. The values of both dictionaries are 2-tuples. For pipeline
inputs, values of the \emph{inputs} dictionary consist of a name of an entry in the data specification
...
upstream node's output interface (output connections to other nodes are implied by input connections to the receiving nodes).
...
Data from group analyses (i.e. with \emph{per\_subject}, \emph{per\_visit}, and
...
do not need to be conventionally named when using basic or XNAT
...
from a previously run workflow as a match for a downstream derivative, although this is
...
derivatives and consists of interface parameter values,
software versions of external requirements, Arcana and Nipype, 
...
are able to quickly prototype analyses that can be deployed on large-scale
...
The framework provided by Arcana should facilitate the development of workflows
that are consistent with F.A.I.R. guiding principles \citep{wilkinson_fair_2016},
particularly with regards to interoperability and reusability.
and their extensible, modular architecture should encourage code reuse. Every derivative created
by arcana is accompanied with sufficient provenance information to
fully reproduce the workflow. Analyses implemented in Arcana are interoperable between different
repository and execution environments. In addition, the implementation of neuroimaging
analyses in Banana is planned to closely follow the specification for BIDS
derivatives \citep{gorgolewski_bids_nodate} to enable interoperability with other tool chains.
Collaboration within a common code-base should help improve the visibility of
workflow implementations.
open source and includes text descriptions and citations of the relevant literature.

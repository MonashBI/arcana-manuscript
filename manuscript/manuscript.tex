...
is a time consuming process. Therefore, it is typically not feasible to invest the additional effort
required to implement analysis workflows in a form that generalises across the various acquisition
parameters, data storage conventions and computing environments in use at different research sites,
which limits the reusability of published workflows.

We present a novel software framework, \emph{Abstraction of Repository-Centric ANAlysis (Arcana)}, which enables
the development of complex, ``end-to-end'' workflows that are adaptable to new analyses and
portable to a wide range of computing infrastructures. Analysis templates for specific image types (e.g. MRI contrast)
are implemented as Python classes that specify potential derivatives and modular pipelines to derive them.
Arcana retrieves data from imaging repositories, which can be BIDS datasets, XNAT instances or plain directories, and
stores selected derivatives back into a repository with provenance data for reuse by subsequent analyses.
Pipelines are constructed using Nipype and can be processed on local or on high performance computing environments,
enabling a smooth transition from locally developed prototypes to large-scale deployment. we have created a framework that handles all the tedious aspects and allows researchers to focus on the analysis design
it is flexible to allow researchers to collaborate
...
vein atlases derived from manual tracings with QSM and SWI
contrasts derived from the T2*-weighted acquisition (Figure \ref{fig:t2star_t1_study}).
...
`coreg\_to\_atlas\_warp' specifications in the T2starStudy, in order to improve the registration.
...
\lstinputlisting[basicstyle=\footnotesize,caption=ArcanaPaper class definition,lastline=135]{../code/arcana_paper.py}
\lstinputlisting[basicstyle=\footnotesize,caption=Application of ArcanaPaper class to a dataset,firstline=139]{../code/arcana_paper.py}

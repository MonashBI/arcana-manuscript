...
\keywords{Neuroimaging \and Workflows \and Repository \and Large-scale \and Reproducibility}
...
HPC environments). In this case, modules are loaded before the interface of a pipeline node
is run based on the requirements specified for the node during construction of the pipeline
...
\emph{T2starwT1Study} MultiStudy class to combine analysis of T2* and
...
The \emph{T2starwT1Study} class implements an algorithm to generate
...
registered to the T1-weighted acquisition \citep{ward_combining_2018} (Figure \ref{fig:t2star_t1_study}).
...
(\textbf{cite Lifespan paper ??}). Methods for Figure \ref{fig:veins}, \ref{fig:fa_acs},
and \ref{fig:tractography} in
...
which can be used as a base for biomedical imaging analysis containers.
...
employed at Monash Biomedical Imaging prior to 2016.
